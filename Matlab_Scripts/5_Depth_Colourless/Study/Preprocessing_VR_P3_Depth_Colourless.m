function Preprocessing_VR_P3_Depth_Colourless(exp)
...
%             EEG = pop_mergeset(EEG1,EEG2,1);
            ...
            %% if participant is blah blah then have one for each condition - else 2 for each condition
            if strcmp('on',exp.filter) == 1
                EEG = pop_eegfilt( EEG,exp.highpass, exp.lowpass, [], 0);
%                 EEG = pop_eegfilt( EEG,[], exp.lowpass, [], 0);
%                 EEG = pop_eegfilt( EEG, exp.highpass,[],[], 0);
            end
           ...
           dead_count_4 = 0;
            ...
            EEG = gratton_emcp(EEG,exp.selection_cards,{'HEOG'},{'VEOG'}); %this assumes the eye channels are called this
