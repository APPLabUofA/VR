...
% Channel_keySet = {'1','15','7','8','9','17','18'};
% Channel_valueSet = {'O1','O2','Pz','Cz','Fz','HEOG','VEOG'};
% Channel_Map = containers.Map(keySet_time,valueSet_time_2);
% !!! remeber to add Channel_Map(z) into plot titles - because this will
% output A LOT of charts - organize logically in a powerpoint grouped by
% condition first (near/far), channel second
% for z = length(Channel_keySet)    
...
waveform =  'P3'; %%'P1','N1','P2','N2','P3' PICK A WAVEFORM!!!
keySet_time = {'P1','N1','P2','N2','P3'};
valueSet_time_1 = {10,40,80,150,350}; 
valueSet_time_2 = {30,60,150,200,550};
Time_Map_1 = containers.Map(keySet_time,valueSet_time_1);
Time_Map_2 = containers.Map(keySet_time,valueSet_time_2);
time_1 = Time_Map_1(waveform);
time_2 = Time_Map_2(waveform);
...
%%% Same thing but from cells instead of a structure
% for i_depth = 1:ndepths
%     erp_depth_{i_depth} = all_chan_erpdata(i_depth,event_1).cond;
%     erp_depth__mean{i_depth} = mean(erp_depth_{i_depth}(:,time_window),2);
%     erp_depth__mean{i_depth}(17:18) = NaN;
% end
...
for i_depth = 1:ndepths
    ...
    if i_depth == target_depth
        ...
        title([titled '_Condition_Standard_Topoplot__' exp.event_names{event_1} '__Depth_' num2str(i_depth)]);
    end %% condense based on a Standard_Target = 
    t = colorbar('peer',gca);
...
%%% Initialize time dictionaries
%%%P1 ~ 10-30ms %%%N1 ~ 40-60ms %%%P2 ~ 80-150ms %%%N2 ~ 150-200ms %%%P3 ~ 300-400ms
waveform =  'P1'; %%'P1','N1','P2','N2','P3' PICK A WAVEFORM!!!
keySet_time = {'P1','N1','P2','N2','P3'};
valueSet_time_1 = {10,40,80,150,350}; 
valueSet_time_2 = {30,60,150,200,550};
Time_Map_1 = containers.Map(keySet_time,valueSet_time_1);
...
for i_depth = 1 : length(Depths)
    ...
    Depthy_Diff.Depths{i_depth} = all_chan_erpdata(target_depth,event_1).cond-all_chan_erpdata(i_depth,event1).cond;
    Depthy_Diff.Depths_mean{i_depth} = mean(Depthy_Diff.Depths{i_depth}(:,time_window),2);
    Depthy_Diff.Depths_mean{i_depth}(17:18) = NaN;
end
...
for i_depth = 1:ndepths
    figure('Color',[1 1 1]);hold on;
    set(gca,'Color',[1 1 1]);
    topoplot(Depthy_Diff.Depths_mean{i_depth},exp.electrode_locs, 'whitebk','on','plotrad',.6,'maplimits',lims)
    title([titled '_Condition_Target_Difference_Topoplot__' exp.event_names{event_1} '__Depth_' num2str(target_depth) ' - Depth_' num2str(i_depth)]);
    t = colorbar('peer',gca);
    set(get(t,'ylabel'),'String', 'Voltage Difference (uV)');
    hold off;
end
...
for i_depth = 1 : length(Depths)
    Depthy.Depth_mean(i_depth) = mean(erpdata(i_depth,event_1).cond(time_window));
    Depthy.Depth_se(i_depth) = mean(std(erpdata_parts(i_depth,event_1).cond(time_window,:),[],2)/nparts);
end

figure;hold on;
bar([Depthy.Depth_mean]);
errorbar([Depthy.Depth_mean],[Depthy.Depth_mean],'.');
title([titled '__Condition_Bar_Graph_' exp.event_names{event_1}])
hold off;
...
%%% Initialize time dictionaries
%%%P1 ~ 10-30ms %%%N1 ~ 40-60ms %%%P2 ~ 80-150ms %%%N2 ~ 150-200ms %%%P3 ~ 300-400ms
waveform =  'N1'; %%'P1','N1','P2','N2','P3' PICK A WAVEFORM!!!
keySet_time = {'P1','N1','P2','N2','P3'};
valueSet_time_1 = {10,40,80,150,350}; 
valueSet_time_2 = {30,60,150,200,550};
Time_Map_1 = containers.Map(keySet_time,valueSet_time_1);
