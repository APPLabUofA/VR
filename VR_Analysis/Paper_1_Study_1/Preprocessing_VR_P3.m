function Preprocessing_VR_P3(exp)
    ...
    if strcmp('on',exp.segments) == 1
        ...
        for i_part = 1:nparts
            ...
            for i_event = 2:allevents %skip the first
                ...
                % % %                     EEG.event(i_event-1).type = 'S999';
                % % %                     if strcmp(exp.participants(i_part), '006') & strcmp(exp.name, 'visual_hs')
                % % %                         EEG.event(i_event-2).type = 'S999';
                % % %                     end
                % % %                 end
                % % %
                % % %                 if i_event == allevents & i_event == 504
                % % %                     EEG.event(i_event).type = 'S999';
                % % %                 end
                
                if strcmp(EEG.event(i_event).type,'S 85')
                    EEG.event(i_event).type = 'S  1';
                elseif strcmp(EEG.event(i_event).type,'S255')
                    EEG.event(i_event).type = 'S  2';
                elseif strcmp(EEG.event(i_event).type,'S153') & strcmp(EEG.event(i_event - 1).type,'2')
%                     EEG.event(i_event).type = 'S  3';
                    correct_rt = correct_rt + 1;
                    trigs_to_remove = [trigs_to_remove,i_event];
                elseif strcmp(EEG.event(i_event).type,'S153') & strcmp(EEG.event(i_event - 1).type,'1')
%                     EEG.event(i_event).type = 'S  4';
                    incorrect_rt = incorrect_rt + 1;
                    trigs_to_remove = [trigs_to_remove,i_event];
                end
...
% %             if correct_rt == 0 & incorrect_rt == 0
% %                 exp.events = {[1],[2]};    %must be matrix (sets x events)
% %                 exp.event_names = {'Low_Tones','High_Tones'}; %name the columns
% %             elseif correct_rt == 0 & incorrect_rt ~= 0
% %                 exp.events = {[1],[2],[4]};    %must be matrix (sets x events)
% %                 exp.event_names = {'Low_Tones','High_Tones','Incorrect_RT'}; %name the columns
% %             elseif correct_rt ~= 0 & incorrect_rt == 0
% %                 exp.events = {[1],[2],[3]};    %must be matrix (sets x events)
% %                 exp.event_names = {'Low_Tones','High_Tones','Correct_RT'}; %name the columns
% %             end
            
%             EEG.event(trigs_to_remove)= [];
