function Preprocessing_Depth_P3(exp)
...
try
    ...
    if strcmp('on',exp.segments) == 1
        ...
        for i_part = 1:nparts
            ...
            %% load a data file
            if exp.preprocess == 5
                if str2num(exp.participants{i_part}) == 5
                    EEG_part1 = pop_loadbv(exp.pathname, [exp.participants{i_part} '_01_depth_p3_near.vhdr']);
                    EEG_part2 = pop_loadbv(exp.pathname, [exp.participants{i_part} '_02_depth_p3_near.vhdr']);
                    EEG_part3 = pop_loadbv(exp.pathname, [exp.participants{i_part} '_depth_p3_far.vhdr']);
                    EEG_part12 = pop_mergeset(EEG_part1,EEG_part2,1);
                    EEG = pop_mergeset(EEG_part12,EEG_part3,1);
                elseif str2num(exp.participants{i_part}) < 11
                    EEG_part1 = pop_loadbv(exp.pathname, [exp.participants{i_part} '_depth_p3_far.vhdr']);
                    EEG_part2 = pop_loadbv(exp.pathname, [exp.participants{i_part} '_depth_p3_near.vhdr']);
                    EEG = pop_mergeset(EEG_part1,EEG_part2,1);
                elseif str2num(exp.participants{i_part}) >= 11
                    EEG_part1 = pop_loadbv(exp.pathname, [exp.participants{i_part} '_01_depth_p3_far.vhdr']);
                    EEG_part2 = pop_loadbv(exp.pathname, [exp.participants{i_part} '_02_depth_p3_far.vhdr']);
                    EEG_part3 = pop_loadbv(exp.pathname, [exp.participants{i_part} '_01_depth_p3_near.vhdr']);
                    EEG_part4 = pop_loadbv(exp.pathname, [exp.participants{i_part} '_02_depth_p3_near.vhdr']);
                    EEG_part12 = pop_mergeset(EEG_part1,EEG_part2,1);
                    EEG_part34 = pop_mergeset(EEG_part3,EEG_part4,1);
                    EEG = pop_mergeset(EEG_part12,EEG_part34,1);
                end
            else
                if strcmp(exp.participants{i_part},'005') == 1 & exp.preprocess == 2
                    EEG_part1 = pop_loadbv(exp.pathname, [exp.participants{i_part} '_01_' exp.name '.vhdr']);
                    EEG_part2 = pop_loadbv(exp.pathname, [exp.participants{i_part} '_02_' exp.name '.vhdr']);
                    EEG = pop_mergeset(EEG_part1,EEG_part2,1);
                elseif str2num(exp.participants{i_part}) >= 11
                    EEG_part1 = pop_loadbv(exp.pathname, [exp.participants{i_part} '_01_' exp.name '.vhdr']);
                    EEG_part2 = pop_loadbv(exp.pathname, [exp.participants{i_part} '_02_' exp.name '.vhdr']);
            ...
            for i_event = 2:allevents %skip the first
                if exp.preprocess == 4
                    if strcmp(EEG.event(i_event-1).type, '139') == 1 & strcmp(EEG.event(i_event).type, 'S138') == 1
                        EEG.event(i_event).type = 'S999';
                    elseif strcmp(EEG.event(i_event-1).type, '138') == 1 & strcmp(EEG.event(i_event).type, 'S139') == 1
                        EEG.event(i_event).type = 'S999';
                    elseif strcmp(EEG.event(i_event-1).type, '139') == 1 & strcmp(EEG.event(i_event).type, 'S139') == 1
                        EEG.event(i_event).type = 'S999';
                    elseif strcmp(EEG.event(i_event-1).type, '138') == 1 & strcmp(EEG.event(i_event).type, 'S138') == 1
                        EEG.event(i_event).type = 'S999';
                    end
                    if strcmp(EEG.event(i_event-1).type, '1') == 1
                        EEG.event(i_event).type = 'S138';
                        EEG.event(i_event-1).type = '111';
                    elseif strcmp(EEG.event(i_event-1).type, '2') == 1
                        EEG.event(i_event).type = 'S139';
                        EEG.event(i_event-1).type = '222';
                    end
                end
                if strcmp(exp.participants{i_part},'005') == 1
                    if strcmp(EEG.event(i_event-1).type, '5') == 1
                        EEG.event(i_event-1).type = '1';
                        if strcmp(EEG.event(i_event).type, 'S142') == 1 | strcmp(EEG.event(i_event).type, 'S143') == 1
                            EEG.event(i_event).type = 'S138';
                        elseif strcmp(EEG.event(i_event+1).type, 'S142') == 1 | strcmp(EEG.event(i_event+1).type, 'S143') == 1
                            EEG.event(i_event+1).type = 'S138';
                        end
                    elseif strcmp(EEG.event(i_event-1).type, '6') == 1
                        EEG.event(i_event-1).type = '2';
                        if strcmp(EEG.event(i_event).type, 'S142') == 1 | strcmp(EEG.event(i_event).type, 'S143') == 1
                            EEG.event(i_event).type = 'S139';
                        elseif strcmp(EEG.event(i_event+1).type, 'S142') == 1 | strcmp(EEG.event(i_event+1).type, 'S143') == 1
